...
def get_macro_pmi(source: str = "official") -> str:
    ...
    try:
        ...
        try:
            ...
            if non_mfg_df is not None and hasattr(non_mfg_df, 'tail') and not (hasattr(non_mfg_df, 'empty') and non_mfg_df.empty):
        ...
        try:
            ...
            if svc_df is not None and hasattr(svc_df, 'tail') and not (hasattr(svc_df, 'empty') and svc_df.empty):
...
def get_macro_interest_rate() -> str:
    ...
    try:
        ...
        if rate_df is not None and hasattr(rate_df, 'tail') and not (hasattr(rate_df, 'empty') and rate_df.empty):
        ...
        try:
            ...
            if shibor_df is not None and hasattr(shibor_df, 'tail') and not (hasattr(shibor_df, 'empty') and shibor_df.empty):
        ...
        try:
            ...
            if shibor_lpr_df is not None and hasattr(shibor_lpr_df, 'tail') and not (hasattr(shibor_lpr_df, 'empty') and shibor_lpr_df.empty):
        ...
        try:
            ...
            if hibor_df is not None and hasattr(hibor_df, 'tail') and not (hasattr(hibor_df, 'empty') and hibor_df.empty):
        ...
        try:
            ...
            if usa_df is not None and hasattr(usa_df, 'tail') and not (hasattr(usa_df, 'empty') and usa_df.empty):
        ...
        try:
            ...
            if euro_df is not None and hasattr(euro_df, 'tail') and not (hasattr(euro_df, 'empty') and euro_df.empty):
        ...
        try:
            ...
            if japan_df is not None and hasattr(japan_df, 'tail') and not (hasattr(japan_df, 'empty') and japan_df.empty):
        ...
        try:
            ...
            if uk_df is not None and hasattr(uk_df, 'tail') and not (hasattr(uk_df, 'empty') and uk_df.empty):
                uk_df = uk_df.tail(3)
                items = []
...
def get_macro_money_supply() -> str:
    ...
    try:
        ...
        for label, df in [("M2货币供应量", m2), ("新增人民币贷款", loan), ("社会融资规模", financing)]:
            if df is not None and hasattr(df, 'tail') and not (hasattr(df, 'empty') and df.empty):
                df = df.tail(3)
                items = []
                if hasattr(df, 'iterrows'):
        ...
        try:
            ...
            if fdi_df is not None and hasattr(fdi_df, 'tail') and not (hasattr(fdi_df, 'empty') and fdi_df.empty):
        ...
        try:
            ...
            if lev_df is not None and hasattr(lev_df, 'tail') and not (hasattr(lev_df, 'empty') and lev_df.empty):
...
def get_market_pe_pb_info(code: str = "000001") -> str:
    ...
    try:
        ...
        if pe_df is not None and hasattr(pe_df, 'tail') and not (hasattr(pe_df, 'empty') and pe_df.empty):
            pe_df = pe_df.tail(5)
            items = []
        ...
        if pb_df is not None and hasattr(pb_df, 'tail') and not (hasattr(pb_df, 'empty') and pb_df.empty):
            pb_df = pb_df.tail(5)
            items = []
    ...
    try:
        ...
        if hasattr(df, 'iterrows'):
            for _, row in df.iterrows():
                ...
                records.append(f"  {date}: 主力净流入={main_inflow}, 超大单={super_inflow}, 大单={big_inflow}")
...
def get_fund_company_aum_rank(limit: int = 20) -> str:
    ...
    try:
        ...
        try:
            ...
            if hist_df is not None and hasattr(hist_df, 'tail') and not (hasattr(hist_df, 'empty') and hist_df.empty):
    ...
    try:
        ...
        if hasattr(df, 'iterrows'):
            for _, row in df.iterrows():
                ...
                records.append(f"  {date}: 机构={inst_ratio}, 个人={retail_ratio}, 内部持有={internal_ratio}")
    ...
    try:
        ...
        if hasattr(df, 'iterrows'):
            for i, (_, row) in enumerate(df.iterrows(), 1):
                ...
                results.append(f"{i}. {name} ({company}): 管理基金数={fund_count}, 管理规模={total_scale}")
    ...
    try:
        ...
        if hasattr(df, 'iterrows'):
            for i, (_, row) in enumerate(df.iterrows(), 1):
                ...
                results.append(f"{i}. {code} {name}: 单位净值={nav}, 累计净值={acc_nav}, 涨跌幅={change}")

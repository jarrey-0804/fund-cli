...
#### 新增数据源
- **QiemanAdapter**: 完整集成Qieman MCP服务器，74个专业接口
- **QiemanMCPClient**: MCP协议客户端，支持JSON-RPC 2.0通信
- **双数据源架构**: AKShare + Qieman MCP 互补，自动注册和降级

#### 基金分析接口 (30个)
- 基金搜索、详情、净值历史、业绩表现
- 基金持仓（含QDII穿透）、基金诊断、组合诊断
- 组合回测、相关性分析、风险分析
- 基金经理信息、业绩、公司信息
- 基金评级、规模历史、持有人结构、风格分析
...
#### 配置项
- `FUND_DATA_QIEMAN_API_KEY`: Qieman API密钥
- `FUND_DATA_QIEMAN_ENABLED`: 启用/禁用Qieman数据源
- `FUND_DATA_QIEMAN_BASE_URL`: MCP服务器地址
- `FUND_DATA_QIEMAN_TIMEOUT`: 请求超时时间
- `FUND_DATA_QIEMAN_MAX_RETRIES`: 最大重试次数

### Changed
- 数据源优先级默认调整为 `qieman,akshare,tushare,wind`
- README更新v3.8.0新特性说明
- 架构图更新，展示Qieman MCP数据源
